...
total_epoches = 200 #500 1.100 2. 200 
train_evaluation_step = 50 #100 # Evaluate on the training batch after some steps.
val_evaluation_epoch = 50 #50  # Evaluate on the whole validation dataset after some epochs. Set to None to evaluate every epoch.
...
lr_lower_bound = 1e-7  # The minimum learning rate.
...
warm_up_epoch = 10  # 3 Warm up training epoches. Set to a larger value if gradient explodes.

### some constants in validation
# nms
nms_threshold = 0.1  #0.45  iou threshold in nms operation
